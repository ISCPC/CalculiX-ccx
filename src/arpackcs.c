  ...
  for(nm=cs[1];nm<=cs[2];++nm){
...
#else
      printf("*ERROR in arpackcs: the HeterSolver library is not linked\n\n");
      FORTRAN(stop,());
#endif
    }
    else if(*isolver==12){
...
#else
      printf("*ERROR in arpackcs: the CG/VE library is not linked\n\n");
      FORTRAN(stop,());
#endif
    }
	...
	if((ido==-1)||(ido==1)){
	  ...
	  if(ido==-1){
	    ...
	    TIMELOG_END(tl, "solver1-1 for arpackcs");
	    ...
	    TIMELOG_END(tl, "solver1-2 for arpackcs");
	    for(jrow=0;jrow<neq[1];jrow++){
	...
	if(neq[1]>0){
	  ...
	  TIMELOG_END(tl, "solver2 for arpackcs");
	}
      }
